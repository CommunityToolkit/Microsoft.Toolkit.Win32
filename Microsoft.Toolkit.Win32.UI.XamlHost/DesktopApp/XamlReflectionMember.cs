...
namespace Microsoft.Toolkit.Win32.UI.XamlHost
{
    ...
    internal class XamlReflectionMember : IXamlMember
    {
        ...
        private bool _isDependencyProperty = false;
        private bool _isReadOnly = false;
        private Type _underlyingType = null; // The type of the property's value
        private Type _declaringType = null; // The type the property was declared on

        // Fields for attachable properties
        private bool _isAttachable = false;
        private Type _targetType = null; // For attached properties, what type the property can be set on - otherwise null
        private MethodInfo _attachableGetterInfo = null; // Method info for the getter method for an attachable property
        private MethodInfo _attachableSetterInfo = null; // Method info for the setter method for an attachable property
        ...
        public static XamlReflectionMember Create(string memberName, Type declaringType)
        {
            ...
            foreach (MethodInfo mi in typeMethods)
            {
                ...
                if (mi.IsStatic && mi.IsPublic)
                {
                    ...
                    if (mi.Name.Equals("Set" + memberName, StringComparison.InvariantCulture))
                    {
                    ...
                    else if (mi.Name.Equals("Get" + memberName, StringComparison.InvariantCulture))
                    {
                        ParameterInfo[] paramInfo = mi.GetParameters();
            ...
            if (underlyingType != null)
            {
                XamlReflectionMember memb = new XamlReflectionMember(
                    memberName,
        ...
        protected XamlReflectionMember(
            string memberName,
        ...
        public IXamlType Type
        {
            get
            {
                return ReflectionXamlMetadataProvider.GetXamlTypeInternal(_underlyingType);
            }
        }
        ...
        public IXamlType TargetType
        {
            get
            {
                return ReflectionXamlMetadataProvider.GetXamlTypeInternal(_targetType);
            }
        }
